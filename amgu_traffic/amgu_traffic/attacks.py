...
class Attacks(object):
    ...
    def run_on_intersections(input, attack, labels):
        ...
        for intersection_id in range(input.size()[1]):
            cpy[:, intersection_id] = attack(
                input[:, intersection_id], labels[intersection_id]
            )
        return cpy * 255

    @staticmethod
    def whitebox_untargeted(
        model: nn.Module, imgs: torch.Tensor, target: torch.Tensor, preprocess
    ):
        model.eval()
