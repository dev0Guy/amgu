...
from abstract import RewardWrapper,Preprocessor
import os
import json
import cityflow
import numpy as np
...
Information = namedtuple("Preprocess", ["eng","road_mapper","state_shape", "intersections", "actionSpaceArray","action_impact", "intersectionNames", "summary"])

class CFWrapper():
    """Class Integrate With Cityflow & Create The Env From JSON File."""
    
    def __init__(self, config: dict, reward_class: RewardWrapper, preprocess_dict: dict):
        # Asserts 
        ...
        assert isinstance(preprocess_dict, dict) and 'func' in preprocess_dict and 'argument_list' in preprocess_dict
        ...
        # distruct arguments 
        self.eng, self.road_mapper, self.org_state_shape, self.intersections, self.actionSpaceArray,self.action_impact, self.intersectionNames, self.summary = self.information
        # define information
    ...
    def _extract_information(self):
        ...
        # define state size
        state_shape = (self.chl_num,len(intersections),(in_lane+out_lane),summary['division'])
        return Information(eng, road_mapper, state_shape, intersections, actionSpaceArray,action_impact, intersectionNames, summary)
    ...
    def step_information(self,action: np.array):
        ...
        if (type(action) != int and type(action) != np.int32 ) and len(action) != len(self.intersectionNames):
