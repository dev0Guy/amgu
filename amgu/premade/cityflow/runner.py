...
from abstract import RunnerWrapper
import os
...
class RayRunner(RunnerWrapper):
    def __init__(self,config: dict, model, env_func: CFWrapper, agent: str):
        ...
        self.config['env_config']['config_path'] = os.path.join(self.run_folder,org_path)
        self.config['env_config']['res_path'] = os.path.join(self.run_folder,self.res_path)
        register_env(config['env'], env_func)
        ModelCatalog.register_custom_model(self.config['model']['custom_model'], self.model)

    
    def train(self, attack=None):
        ray.tune.run(self.agent,config=self.config,local_dir=self.res_path,
                    checkpoint_at_end=True,mode="min",stop=self.stop)
