...
class SingleIntersection(SingleAgentCityFlow):
    
    def __init__(self, config: dict):
        super().__init__(config)
        SingleAgentCityFlow.__init__(self,config)
        self.observation_space: gym.spaces = gym.spaces.Box(np.zeros(self.state_shape),np.zeros(self.state_shape)+255,dtype=np.float64) 
...
# class MultiAgentCityFlow(MultiAgentEnv,_Wrapper): 
